...
const convertForm = (idForm, radios) => {
    ...
    radios.forEach(radioName => {
        const selecionado = form.querySelector(`input[name="${radioName}"]:checked`);
        if (selecionado) {
            inpValue[radioName] = {
                type: 'radio',
...
const addItemToListPrevious = (idInputs, id) => {
    ...
    if (ItemList.nome.modif === '') {
        ...
        document.querySelector(`#preVisualizacao${id}`).innerHTML = 'Pré-Visualização'
        Object.keys(ItemList).forEach(key => {
            try {
...
const limparList = async () => {
    ...
    if (!confirm(`Deseja realmente limpar toda a lista de OS?`)) return;
    loading('open')
